 ...
 */
static inline const char *arch_debug_get_cpu_mode_str(uint8_t mode) {
...
static inline char *arch_debug_get_psr_str(regpsr_t psr, char *buf, size_t buflen) {
    snprintf(buf, buflen, "%c%c%c%c%c%c%c%c %s", psr.b.n ? 'N' : '.', psr.b.z ? 'Z' : '.', psr.b.c ? 'C' : '.',
            psr.b.v ? 'V' : '.', psr.b.q ? 'Q' : '.', psr.b.async_abort ? '.' : 'A',
            psr.b.irq ? '.' : 'I', psr.b.fiq ? '.' : 'F', arch_debug_get_cpu_mode_str(psr.b.mode));
    return buf;
}

static inline char *arch_debug_get_psr_str_from_ctx(spctx_t *ctx, char *buf, size_t buflen) {
    return arch_debug_get_psr_str(ctx->spsr, buf, buflen);
}

static inline void arch_debug_dump_regs(const int32_t *regs, uint8_t nregs, int32_t pc, int32_t lr, regpsr_t cpsr) {
    regpsr_t v = cpsr;
    v.v = 1;

    char cpsrbuf[64] = { 0 };
    error_async("Registers:");
    error_async("   pc=0x%08lx lr=0x%08lx cpsr=0x%08lx (%s mode)", pc, lr, cpsr.v, arch_debug_get_psr_str(cpsr, cpsrbuf, sizeof(cpsrbuf)));
 ...
 */
__attribute__((always_inline)) static inline void arch_debug_dump_all_regs(void) {
    ...
    asm("mrs r4, cpsr");
    asm("push {r4}");
    void (*f)(void) = (void (*)(void)) arch_debug_dump_regs;
    f();
    asm("pop {r4}");
