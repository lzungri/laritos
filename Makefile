...
$(OUT)/laritos.img: Makefile $(OUT) $(OUT)/start.o $(OUT)/vectors.o $(OUT)/entry.o $(SRC_BOARD)/os_memmap.lds
	$(CROSS_COMPILE)ld -T $(SRC_BOARD)/os_memmap.lds $(OUT)/start.o $(OUT)/vectors.o $(OUT)/entry.o -o $(OUT)/laritos.elf
...
printmap: $(OUT)/laritos.img
	$(CROSS_COMPILE)ld --print-map $(OUT)/laritos.elf -T $(SRC_BOARD)/os_memmap.lds
