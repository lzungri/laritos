...
static inline void sched_move_to_ready(pcb_t *pcb) {
    verbose_async("PID %u: %s -> READY", pcb->pid, get_pcb_status_str(pcb->sched.status));
    // TODO Mutex
...
static inline void sched_move_to_running(pcb_t *pcb) {
    verbose_async("PID %u: %s -> RUNNING", pcb->pid, get_pcb_status_str(pcb->sched.status));
    // TODO Mutex
...
static inline void sched_remove_from_zombie(pcb_t *pcb) {
    ...
    verbose_async("PID %u: %s -> NOT_INIT", pcb->pid, get_pcb_status_str(pcb->sched.status));
    // TODO Mutex
    list_del_init(&pcb->sched.sched_node);
...
static inline void sched_move_to_zombie(pcb_t *pcb) {
    verbose_async("PID %u: %s -> ZOMBIE ", pcb->pid, get_pcb_status_str(pcb->sched.status));
    // TODO Mutex
